...
public /*final*/ class WordComposer {
    ...
    /*private*/protected CombinerChain mCombinerChain;
    ...
    /*private*/protected boolean mIsResumed;
    ...
    public WordComposer() {
        ...
        //AnhNDd:
        mCombinerChain = createCombinerChain("");
    ...
    public void restartCombining(final String combiningSpec) {
        ...
        if (!nonNullCombiningSpec.equals(mCombiningSpec)) {
            ...
            //AnhNDd:
            Log.d("phongnve", "restartCombining: "+nonNullCombiningSpec);
            mCombinerChain = createCombinerChain(nonNullCombiningSpec);
    ...
    public void applyProcessedEvent(final Event event) {
        ...
        if (Constants.CODE_DELETE != event.mKeyCode && shouldAddIndex(event)) { //Bkav AnhNDd
    ...
    public LastComposedWord commitWord(final int type, final CharSequence committedWord,
            final String separatorString, final NgramContext ngramContext, int flagAutoCorrect) { //Bkav AnhNDd: thêm flagAutoCorrect
        ...
        //AnhNDd:
        final LastComposedWord lastComposedWord = createLastComposedWord(mEvents,
                mInputPointers, mTypedWordCache.toString(), committedWord, separatorString,
                ngramContext, mCapitalizedMode, flagAutoCorrect);
    ...
    /**
     * AnhNDd: khởi tạo combinerchain
     */
    public CombinerChain createCombinerChain(String nonNullCombiningSpec){
        if(nonNullCombiningSpec.isEmpty()) {
            nonNullCombiningSpec = mCombinerChain.getComposingWordWithCombiningFeedback().toString();
        }
        return new CombinerChain(nonNullCombiningSpec);
    }

    /**
     * AnhNDd: kiểm tra xem kí tự đầu tiên gõ có phải là chữ hoa
     */
    ...
    /**
     * AnhNDd: xét lại từ autocorrection
     */
    public void resetAutoCorrection(){
        mAutoCorrection = null;
    }

    public LastComposedWord createLastComposedWord(final ArrayList<Event> events,
                                                   final InputPointers inputPointers, final String typedWord,
                                                   final CharSequence committedWord, final String separatorString,
                                                   final NgramContext ngramContext, final int capitalizedMode, int flagAutoCorrect) {
        return new LastComposedWord(events, inputPointers, typedWord, committedWord, separatorString, ngramContext, capitalizedMode);
    }

    //Bkav AnhNDd
    public boolean shouldAddIndex(Event event) {
        return true;
    }
}
