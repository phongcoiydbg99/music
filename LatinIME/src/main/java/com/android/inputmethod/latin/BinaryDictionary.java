...
public final class BinaryDictionary extends Dictionary {
    ...
    public static final int DICTIONARY_MAX_WORD_LENGTH = 572; //AnhNDd: 572
    ...
    private static native boolean updateEntriesForWordWithNgramContextNative(long dict,
            int[][] prevWordCodePointArrays, boolean[] isBeginningOfSentenceArray,
            int[] word, boolean isValidWord, int count, int timestamp, int flagAutoCorrect);
    ...
    public ArrayList<SuggestedWordInfo> getSuggestions(final ComposedData composedData,
        ...
        //AnhNDd: một số cờ để search ở lib.so
        session.mNativeSuggestOptions.setNotUseFullSearch(false);
        session.mNativeSuggestOptions.setUseDictAutocorrect(false);
        if (mLocale.getLanguage().equals("vi")) {
            session.mNativeSuggestOptions.setNotUseFullSearch(true);
            session.mNativeSuggestOptions.setUseDictAutocorrect(false);
        }
        if (settingsValuesForSuggestion.mUseDictAutocorrect) {
            session.mNativeSuggestOptions.setNotUseFullSearch(false);
            session.mNativeSuggestOptions.setUseDictAutocorrect(true);
        }
        boolean isEmojiDict = mDictType.equals("emoji");
        //-----------
        ...
        for (int j = 0; j < count; ++j){
            ...
            if (len > 0) {
                //AnhNDd: với từ điển emoji thì thì thêm kind vào để xử lý đặc biệt riêng
                //TODO: Thực tế thì cần thêm thông tin này ở trong từ điển, nhưng không hiểu rõ cách build từ điển nên chưa thêm như thế được
                int kindAndFlags = session.mOutputTypes[j];
                if(isEmojiDict) kindAndFlags |= SuggestedWordInfo.KIND_EMOJI;

                suggestions.add(new SuggestedWordInfo(
                        new String(session.mOutputCodePoints, start, len),
                        "" /* prevWordsContext */,
                        (int)(session.mOutputScores[j] * weightForLocale),
                        /*session.mOutputTypes[j]*/kindAndFlags,
                        this /* sourceDict */,
    ...
    public boolean updateEntriesForWordWithNgramContext(@Nonnull final NgramContext ngramContext,
        ...
        mHasUpdated = true;
        return true;*/
        return updateEntriesForWordWithNgramContext(ngramContext,word,isValidWord,count,timestamp,0);
    }

    //AnhNDd
    public boolean updateEntriesForWordWithNgramContext(@Nonnull final NgramContext ngramContext, final String word, final boolean isValidWord,
                                                        final int count, final int timestamp, int flagAutoCorrect) {
        if (TextUtils.isEmpty(word)) {
            return false;
        }
        final int[][] prevWordCodePointArrays = new int[ngramContext.getPrevWordCount()][];
        ...
        final int[] wordCodePoints = StringUtils.toCodePointArray(word);
        if (!updateEntriesForWordWithNgramContextNative(mNativeDict, prevWordCodePointArrays,
                isBeginningOfSentenceArray, wordCodePoints, isValidWord, count, timestamp, flagAutoCorrect)) {
            return false;
        }
        mHasUpdated = true;
        return true;
    ...
    public boolean needsToRunGC(final boolean mindsBlockByGC) {
        ...
        //Bkav AnhNDd: Với từ điển UserHistoryTypedDictionary và BkavCacheUserHistoryDictionary thì không cần flush vời GC vì làm như thế là xóa từ
        //điển và xóa file đi
        if (mDictType.equals(TYPE_USER_HISTORY_TYPED) || mDictType.equals(TYPE_CACHE_USER_HISTORY))
            return false;
    ...
    public static final int DICTIONARY_MAX_WORD_SHORT_CUT_LENGTH = 572;

    private static native boolean isInDictionaryNative(long dict, int[] word);

    /**
     * AnhNDd: Gọi xuống native để xử lý lấy các node có thể gõ tiếp theo và điển của từng node
     */
    private static native void getNextNodeSuggestedNative(
            long dict, long traverseSession, int[] inputCodePoints, int inputSize, int[] suggestOptions,
            int[][] prevWordCodePointArrays, boolean[] isBeginningOfSentenceArray,
            int prevWordCount, int[] outputNodeSuggestionCount, int[] outputNodeCodePoints, int[] outputNodeScores);

    /**
     * AnhNDd: add unigram với shortcur của nó
     */
    public boolean addUnigramEntry(
            final String word, final int probability,final String shorcut,final int probabilityShorcut, final boolean isBeginningOfSentence,
            final boolean isNotAWord, final boolean isPossiblyOffensive, final int timestamp) {
        if (word == null || (word.isEmpty() && !isBeginningOfSentence)) {
            return false;
        }
        final int[] codePoints = StringUtils.toCodePointArray(word);
        final int[] codePointsShortcut = StringUtils.toCodePointArray(shorcut);
        if (!addUnigramEntryNative(mNativeDict, codePoints, probability,
                codePointsShortcut, probabilityShorcut,
                isBeginningOfSentence, isNotAWord, isPossiblyOffensive, timestamp)) {
            return false;
        }
        mHasUpdated = true;
    ...
    /**
     * AnhNDd: Lấy ra những node có thể gõ tiếp theo
     */
    public ArrayList<Node> getNextNodeSuggested(final ComposedData composedDataTyped,
        ...
        final int inputSize;
        inputSize =
                composedDataTyped.copyCodePointsExceptTrailingSingleQuotesAndReturnCodePointCount(
                        session.mInputCodePoints);
        if (inputSize < 0) {
            return null;
        }
        session.mNativeSuggestOptions.setUseFullEditDistance(mUseFullEditDistance);
        session.mNativeSuggestOptions.setIsGesture(false);
        session.mNativeSuggestOptions.setBlockOffensiveWords(
                settingsValuesForSuggestion.mBlockPotentiallyOffensive);
        final int[] outputNodeSuggestionCount = new int[1];
        final int[] outputNodeCodePoints = new int[18];
        final int[] outputNodeScores = new int[18];
        getNextNodeSuggestedNative(mNativeDict, getTraverseSession(sessionId).getSession(), session.mInputCodePoints, inputSize,
                session.mNativeSuggestOptions.getOptions(), session.mPrevWordCodePointArrays,
                session.mIsBeginningOfSentenceArray, ngramContext.getPrevWordCount(),
                outputNodeSuggestionCount, outputNodeCodePoints, outputNodeScores);
        ArrayList<Node> nextNodes = new ArrayList<>();
        for (int i = 0; i < outputNodeSuggestionCount[0]; i++) {
            Node nextNode = new Node((char) outputNodeCodePoints[i], outputNodeScores[i]);
    ...
    @Override
    public boolean hasInDictionary(String word) {
        if (TextUtils.isEmpty(word)) {
            return false;
        }
        final int[] codePoints = StringUtils.toCodePointArray(word);
        return isInDictionaryNative(mNativeDict,codePoints);
    }
}
