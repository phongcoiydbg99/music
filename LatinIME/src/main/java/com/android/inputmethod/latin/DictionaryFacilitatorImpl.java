...
import com.android.inputmethod.latin.personalization.BkavCacheUserHistoryDictionary;
import com.android.inputmethod.latin.personalization.UserHistoryDictionary;
import com.android.inputmethod.latin.personalization.UserHistoryTypedDictionary;
import com.android.inputmethod.latin.settings.SettingsValuesForSuggestion;
import com.android.inputmethod.latin.utils.ExecutorUtils;
import com.android.inputmethod.latin.utils.SuggestionResults;
import com.bkav.BkavAutoCorrectBinaryDictionnary;
import com.bkav.BkavUserBinaryDictionary;
...
public class DictionaryFacilitatorImpl implements DictionaryFacilitator {
    ...
    /*private*/protected DictionaryGroup mDictionaryGroup = new DictionaryGroup();
    private volatile CountDownLatch mLatchForWaitingLoadingMainDictionaries = new CountDownLatch(0);
    // To synchronize assigning mDictionaryGroup to ensure closing dictionaries.
    /*private*/protected final Object mLock = new Object();
    ...
    static {
        ...
        //AnhNDd
        DICT_TYPE_TO_CLASS.put(Dictionary.TYPE_USER_HISTORY_TYPED, UserHistoryTypedDictionary.class);
        DICT_TYPE_TO_CLASS.put(Dictionary.TYPE_CACHE_USER_HISTORY, BkavCacheUserHistoryDictionary.class);
        DICT_TYPE_TO_CLASS.put(Dictionary.TYPE_MAIN_VI_AUTOCORRECT, BkavAutoCorrectBinaryDictionnary.class);
        DICT_TYPE_TO_CLASS.put(Dictionary.TYPE_USER, BkavUserBinaryDictionary.class);
    }
     ...
     */
    /*private*/protected static class DictionaryGroup {
    ...
    public void resetDictionaries(
        ...
        //AnhNDd:
        addSubDictsUserHistoryTyped(subDictTypesToUse,newLocale);
    ...
    void doReloadUninitializedMainDictionaries(final Context context, final Locale locale,
        ...
        //        //AnhNDd:
        final Dictionary mainDict = createMainDict(context, locale);
    ...
    public void resetDictionariesForTesting(final Context context, final Locale locale,
        ...
        for (final String dictType : dictionaryTypes) {
            if (dictType.equals(Dictionary.TYPE_MAIN)) {
                ...
                //        locale);
                //AnhNDd:
                mainDictionary = createMainDict(context, locale);
            } else {
    ...
    /*private*/protected int getFrequency(final String word) {
        if (TextUtils.isEmpty(word)) {
    ...
    /*private*/protected boolean clearSubDictionary(final String dictName) {
        final ExpandableBinaryDictionary dictionary = mDictionaryGroup.getSubDict(dictName);
        if (dictionary == null) {
            return false;
        }
    ...
    /**
     * AnhNDd: Thực hiện tạo mới từ điển main cho ngôn ngữ
     */
    public Dictionary createMainDict(Context context, Locale locale) {
        return DictionaryFactory.createMainDictionaryFromManager(context,
                locale);
    }

    public void addSubDictsUserHistoryTyped(final HashSet<String> subDictTypesToUse, final Locale locale) {
        //AnhNDd: chỉ thêm với locale vi
        if (locale.getLanguage().equals("vi")) {
            subDictTypesToUse.add(Dictionary.TYPE_USER_HISTORY_TYPED);
            subDictTypesToUse.add(Dictionary.TYPE_CACHE_USER_HISTORY);
            subDictTypesToUse.add(Dictionary.TYPE_MAIN_VI_AUTOCORRECT); //TODO: từ điển này lớn cần chuyển vào luồng như từ điển main?
            subDictTypesToUse.add(Dictionary.TYPE_EMOJI);
            subDictTypesToUse.add(Dictionary.TYPE_BASE_VN);
        }
    }

    //Bkav AnhNDd: custom lại thêm kiểu từ điển để lấy ra
    public void addToUserHistory(String dictType, final String suggestion, final boolean wasAutoCapitalized,
                                 @Nonnull final NgramContext ngramContext, final long timeStampInSeconds,
                                 final boolean blockPotentiallyOffensive, final int flagAutoCorrect){
        // Update the spelling cache before learning. Words that are not yet added to user history
        // and appear in no other language model are not considered valid.
        putWordIntoValidSpellingWordCache("addToUserHistory", suggestion);
        ...
        for (int i = 0; i < words.length; i++) {
            final String currentWord = words[i];
            final boolean wasCurrentWordAutoCapitalized = (i == 0) ? wasAutoCapitalized : false;
            addWordToUserHistory(dictType, mDictionaryGroup, ngramContextForCurrentWord, currentWord,
                    wasCurrentWordAutoCapitalized, (int) timeStampInSeconds,
                    blockPotentiallyOffensive, flagAutoCorrect);
            ngramContextForCurrentWord =
                    ngramContextForCurrentWord.getNextNgramContext(new WordInfo(currentWord));
        }
    }

    private void addWordToUserHistory(String type, final DictionaryGroup dictionaryGroup,
                                      final NgramContext ngramContext, final String word, final boolean wasAutoCapitalized,
                                      final int timeStampInSeconds, final boolean blockPotentiallyOffensive, int flagAutoCorrect) {
        final ExpandableBinaryDictionary userHistoryDictionary =
                dictionaryGroup.getSubDict(/*Dictionary.TYPE_USER_HISTORY*/type);
        if (userHistoryDictionary == null || !isForLocale(userHistoryDictionary.mLocale)) {
            return;
        }
        final int maxFreq = getFrequency(word);
        ...
        if(type.equals(Dictionary.TYPE_CACHE_USER_HISTORY)){
            //AnhNDd: Với từ điển cache thì lưu lại hết không cần sét thêm trường hợp
            final boolean isValid = maxFreq > 0;
            UserHistoryDictionary.addToDictionary(userHistoryDictionary, ngramContext, word,
                    isValid, timeStampInSeconds, flagAutoCorrect);
            return;
        }
        final String lowerCasedWord = word.toLowerCase(dictionaryGroup.mLocale);
        final String secondWord;
