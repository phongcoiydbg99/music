...
import com.bkav.BtalkLogger;
import com.bkav.NextNodeResults;
 ...
 */
    ...
    /*private*/public void removeBinaryDictionary() { //AnhNDd: chuyển sang public lưu ý không sử dụng linh tinh
        asyncExecuteTaskWithWriteLock(new Runnable() {
            @Override
            public void run() {
        ...
        asyncExecuteTaskWithWriteLock(new Runnable() {
            ...
            public void run() {
                ...
                if (!binaryDictionary.removeUnigramEntry(word)) {
                    if (DEBUG) {
                        Log.w(TAG, "Cannot remove unigram entry: " + word);
                    }
                }
            }
        });
    ...
    public void updateEntriesForWord(@Nonnull final NgramContext ngramContext,
        ...
        updateEntriesForWord(ngramContext,word,isValidWord,count,timestamp, 0);
    }
    ...
    /**
     * AnhNDd: Với từ điển lịch sử người dùng, sẽ lấy ra các node có thể gõ tiếp theo ở đó
     * ở đây để check lock
     */
    public ArrayList<Node> getNextNodeSuggested(final ComposedData composedDataTyped, final ComposedData composedData,
        ...
        try {
            ...
            if (lockAcquired) {
                ...
                ArrayList<Node> nextNodes = mBinaryDictionary.getNextNodeSuggested(composedDataTyped, composedData,ngramContext,settingsValuesForSuggestion,sessionId);
                if (mBinaryDictionary.isCorrupted()) {
                    Log.i(TAG, "Dictionary (" + mDictName +") is corrupted. "
                            + "Remove and regenerate it.");
                    removeBinaryDictionary();
    ...
    //AnhNDd: add unigram với shortcut
    public void addUnigramEntry(final String word, final int frequency,final String shortcut, final int frequencyShortcut,
                                final boolean isNotAWord, final boolean isPossiblyOffensive, final int timestamp) {
        updateDictionaryWithWriteLock(new Runnable() {
            @Override
            public void run() {
                addUnigramLocked(word, frequency, shortcut, frequencyShortcut, isNotAWord, isPossiblyOffensive, timestamp);
            }
        });
    }

    protected void addUnigramLocked(final String word, final int frequency, final String shortcut, final int frequencyShortcut,
                                    final boolean isNotAWord, final boolean isPossiblyOffensive, final int timestamp) {
        if (!mBinaryDictionary.addUnigramEntry(word, frequency,shortcut,frequencyShortcut,
                false /* isBeginningOfSentence */, isNotAWord, isPossiblyOffensive, timestamp)) {
            Log.e(TAG, "Cannot add unigram entry. word: " + word);
        }
    }
    ...
    //AnhNDd:
    public void updateEntriesForWord(@Nonnull final NgramContext ngramContext,
                                     final String word, final boolean isValidWord, final int count, final int timestamp, final int flagAutoCorrect) {
        updateDictionaryWithWriteLock(new Runnable() {
            @Override
            public void run() {
                final BinaryDictionary binaryDictionary = getBinaryDictionary();
                ...
                if (!binaryDictionary.updateEntriesForWordWithNgramContext(ngramContext, word,
                        isValidWord, count, timestamp, flagAutoCorrect)) {
                    if (DEBUG) {
                        Log.e(TAG, "Cannot update counter. word: " + word
                                + " context: " + ngramContext.toString());
                    }
    ...
    @Override
    public boolean hasInDictionary(final String word) {
        reloadDictionaryIfRequired();
        boolean lockAcquired = false;
        try {
            lockAcquired = mLock.readLock().tryLock(
            ...
            if (lockAcquired) {
                ...
                return mBinaryDictionary.hasInDictionary(word);
            }
        } catch (final InterruptedException e) {
            Log.e(TAG, "Interrupted tryLock() in isInDictionary().", e);
        } finally {
