...
public class UserDictionaryAddWordContents {
    ...
    //Bkav AnhNDd: lọc những kí tự cho phép
    private InputFilter mInputFilter = new InputFilter() {
        @Override
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
            for (int i = start;i < end;i++) {
                if (!Constants.isCodeVN(source.charAt(i)) && !Constants.isNumber(source.charAt(i)))
                {
                    return "";
                }
            }
    ...
    /* package */ UserDictionaryAddWordContents(final View view, final Bundle args) {
        ...
        //AnhNDd
        mShortcutEditText.setFilters(new InputFilter[]{mInputFilter});
