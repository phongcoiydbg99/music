...
public class LoginAccountUtils {
    ...
    public static String[] getAccountsForLogin(final Context context) {
        //AnhNDd: thực hiện lấy các acount google
        AccountManager manager = (AccountManager) context.getSystemService(ACCOUNT_SERVICE);
        Account[] listAccounts = manager.getAccountsByType("com.google");
        if (listAccounts.length > 0) {
            String[] getName = new String[listAccounts.length];
            for (int i = 0; i < listAccounts.length; i++) {
                getName[i] = listAccounts[i].name;
            }
            return getName;
        }
        return new String[0];
    ...
    //AnhNDd: lớp lày cũng chưa có gì, code luôn vào đâ
}
