...
import static com.android.inputmethod.latin.common.Constants.CODE_SPERATOR;
...
public /*final*/ class InputLogic {
    ...
    /*private*/protected final SuggestionStripViewAccessor mSuggestionStripViewAccessor;

    // Never null.
    /*private*/protected InputLogicHandler mInputLogicHandler = InputLogicHandler.NULL_HANDLER;

    // TODO : make all these fields private as soon as possible.
    // Current space state of the input method. This can be any of the above constants.
    /*private*/protected int mSpaceState;
    // Never null
    public SuggestedWords mSuggestedWords = SuggestedWords.getEmptyInstance();
    public final Suggest mSuggest;
    /*private*/protected final DictionaryFacilitator mDictionaryFacilitator;
    ...
    /*private*/protected int mDeleteCount;
    ...
    public InputLogic(final LatinIME latinIME,
        ...
        //AnhNDd:
        mWordComposer = createWordComposer();

        //mConnection = new RichInputConnection(latinIME);
        //AnhNDd:
        mConnection = createRichInputConnection(latinIME);

        mInputLogicHandler = InputLogicHandler.NULL_HANDLER;
        //mSuggest = new Suggest(dictionaryFacilitator);
        //AnhNDd:
        mSuggest = createSuggest(dictionaryFacilitator);
    ...
    public void startInput(final String combiningSpec, final SettingsValues settingsValues) {
        ...
        if (InputLogicHandler.NULL_HANDLER == mInputLogicHandler) {
            ...
            //AnhNDd:
            mInputLogicHandler = createInputLogicHandler(mLatinIME,this);
        } else {
        ...
        if (settingsValues.mShouldShowLxxSuggestionUi) {
            mConnection.requestCursorUpdates(true /* enableMonitor */,
                    false /* requestImmediateCallback */);
        }
    }
    ...
    public InputTransaction onPickSuggestionManually(final SettingsValues settingsValues,
        ...
        //AnhNDd: Với từ điển emoji thì không cần update lại gợi ý, để cho người dùng lựa chọn emoji thoải mái
        if (!suggestionInfo.isEmoji()) {
            handleInsertAutomaticSpaceWhenPickSuggestion(settingsValues);
    ...
    public InputTransaction onCodeInput(final SettingsValues settingsValues,
        ...
        Log.d("phongnve", "onCodeInput2: "+ processedEvent.mCodePoint);
        final InputTransaction inputTransaction = new InputTransaction(settingsValues,
    ...
    public void setSuggestedWords(final SuggestedWords suggestedWords) {
        if (!suggestedWords.isEmpty()) {
            ...
            setAutoCorrection(suggestedWordInfo);
        }
     ...
     */
    /*private*/protected void handleConsumedEvent(final Event event, final InputTransaction inputTransaction) {
     ...
     * @param inputTransaction The transaction in progress.
     */
    /*private*/protected void handleFunctionalEvent(final Event event, final InputTransaction inputTransaction,
            final int currentKeyboardScriptId, final LatinIME.UIHandler handler) {
        Log.d("phongnve", "handleFunctionalEvent: "+event.mKeyCode);
     ...
     * @param inputTransaction The transaction in progress.
     */
    /*private*/protected void handleNonFunctionalEvent(final Event event,
            final InputTransaction inputTransaction,
            final LatinIME.UIHandler handler) {
        inputTransaction.setDidAffectContents();
        Log.d("phongnve", "handleNonFunctionalEvent: "+event.mKeyCode);
    ...
    private void handleNonSpecialCharacterEvent(final Event event,
        ...
        if (inputTransaction.mSettingsValues.isWordSeparator(codePoint)
            ...
            Log.d("phongnve", "handleNonSpecialCharacterEvent: ");
     ...
     * @param inputTransaction The transaction in progress.
     */
    /*private*/protected void handleNonSeparatorEvent(final Event event, final SettingsValues settingsValues,
            final InputTransaction inputTransaction) {
        final int codePoint = event.mCodePoint;
     ...
     * @param inputTransaction The transaction in progress.
     */
    /*private*/protected void handleSeparatorEvent(final Event event, final InputTransaction inputTransaction,
            final LatinIME.UIHandler handler) {
        final int codePoint = event.mCodePoint;
        ...
        if (mWordComposer.isComposingWord()) {
            if (settingsValues.mAutoCorrectionEnabledPerUserSettings) {
                Log.d("phongnve", "handleSeparatorEvent: ");
                final String separator = shouldAvoidSendingCode ? LastComposedWord.NOT_A_SEPARATOR
                        : StringUtils.newSingleCodePointString(codePoint);
                commitCurrentAutoCorrection(settingsValues, separator, handler);
                inputTransaction.setDidAutoCorrect();
            } else {
                Log.d("phongnve", "handleSeparatorEvent1: ");
     ...
     * @param inputTransaction The transaction in progress.
     */
    /*private*/protected void handleBackspaceEvent(final Event event, final InputTransaction inputTransaction,
        ...
        if (mWordComposer.isComposingWord()) {
            ...
            if (mConnection.hasSelection()) {
                ...
                if (inputTransaction.mSettingsValues.isBeforeJellyBean()
                    ...
                    //AnhNDd:
                    final int lengthToDelete = getLengthToDelete(codePointBeforeCursor);
                    ...
                    if (mDeleteCount > Constants.DELETE_ACCELERATE_AT) {
                        ...
                        if (codePointBeforeCursorToDeleteAgain != Constants.NOT_A_CODE) {
                            ...
                            //AnhNDd:
                            final int lengthToDeleteAgain = getLengthToDeleteAgain(codePointBeforeCursorToDeleteAgain);
...
//            }
            //AnhNDd:
            updateSuggetOnBackPress(inputTransaction,currentKeyboardScriptId);
        }
    }
    ...
    /*private*/public void performAdditionToUserHistoryDictionary(final SettingsValues settingsValues,
        ...
        //AnhNDd:
        if(!checkAutoCorrectionEnabled(settingsValues)) return;
        ...
        //AnhNDd: Thêm từ vào từ điển cache
        mDictionaryFacilitator.addToUserHistory(Dictionary.TYPE_CACHE_USER_HISTORY,suggestion, wasAutoCapitalized,
                ngramContext, timeStampInSeconds, settingsValues.mBlockPotentiallyOffensive);
        //Bkav AnhNDd: Thêm từ vào từ điển gõ telex nhưng chưa dùng
        addToUserHistoryTyped(ngramContext);*/
                ...
                new OnGetSuggestedWordsCallback() {
                    ...
                    public void onGetSuggestedWords(final SuggestedWords suggestedWords) {
                        ...
                        if (suggestedWords.size() > 1 || typedWordString.length() <= 1) {
                            ...
                            //AnhNDd: nếu không lấy ra được gì ý gì thì không hiển thị gì nữa
                            holder.set(SuggestedWords.getEmptyInstance());
                        }
                    }
                }
        );
     ...
     * @param settingsValues the current values of the settings.
     */
    /*private*/protected void revertCommit(final InputTransaction inputTransaction,
        ...
        //AnhNDd:
        addToUserHistoryTyped(mLastComposedWord);
     ...
     */
    /*private*/protected int getActualCapsMode(final SettingsValues settingsValues,
     ...
     */
    /*private*/protected EditorInfo getCurrentInputEditorInfo() {
     ...
     */
    /*private*/protected void onSettingsKeyPressed() {
    ...
    private void resetEntireInputState(final int newSelStart, final int newSelEnd,
        ...
        //AnhNDd:
        final boolean shouldFinishComposition = shouldFinishComposition();
    ...
    private void sendDownUpKeyEvent(final int keyCode) {
        Log.d("phongnve", "sendDownUpKeyEvent: ");
    ...
    /*private*/protected void sendKeyCodePoint(final SettingsValues settingsValues, final int codePoint) {
     ...
     * @param settingsValues the current values of the settings.
     */
    /*private*/protected void insertAutomaticSpaceIfOptionsAndTextAllow(final SettingsValues settingsValues) {
    ...
    private void commitCurrentAutoCorrection(final SettingsValues settingsValues,
        ...
        //AnhNDd:
        if(checkWordTypedInMainVi(autoCorrectionOrNull)) autoCorrectionOrNull = null;

        //Bkav AnhNDd: Nếu autocorrect khác null nghĩa là đó là từ sửa đúng khi thêm vào từ điển sẽ add flag vào
        int flagAutoCorrect = autoCorrectionOrNull != null ? Constants.FLAG_AUTOCORRECT_RIGHT : 0;

        final String typedWord = mWordComposer.getTypedWord();
        /*final*/ String stringToCommit = (autoCorrectionOrNull != null)
                ? autoCorrectionOrNull.mWord : typedWord;
        //Bkav AnhNDd
        stringToCommit = preCommitAutocorrect(typedWord, stringToCommit);
        ...
        if (stringToCommit != null) {
            ...
            commitChosenWord(settingsValues, stringToCommit,
                    LastComposedWord.COMMIT_TYPE_DECIDED_WORD, separator, flagAutoCorrect); //AnhNDd them flagAutoCorrect
     ...
     * @param separatorString the separator that's causing the commit, or NOT_A_SEPARATOR if none.
     */
    /*private*/protected void commitChosenWord(final SettingsValues settingsValues, final String chosenWord,
        ...
        commitChosenWord(settingsValues,chosenWord,commitType,separatorString,0);
    }
     ...
     *call
     * <p>Currently using this method is optional and you can still directly
     ...
     */
    /*private*/protected void setComposingTextInternalWithBackgroundColor(final CharSequence newComposingText,
    ...
    /**
     * AnhNDd: khởi tạo wordcomposer
     */
    public WordComposer createWordComposer(){
        return new WordComposer();
    }

    /**
     * AnhNDd: khởi tạo InputLogicHandler
     */
    public InputLogicHandler createInputLogicHandler(final LatinIME latinIME, final InputLogic inputLogic){
        return new InputLogicHandler(latinIME, inputLogic);
    }

    public Suggest createSuggest(final DictionaryFacilitator dictionaryFacilitator){
        return new Suggest(dictionaryFacilitator);
    }

    public RichInputConnection createRichInputConnection(LatinIME latinIME){
        return new RichInputConnection(latinIME);
    }

    /**
     * AnhNDd: thông báo để cập nhật biến
     */
    public void setShouldUpdateWord(boolean bool){

    }

    /**
     * AnhNDd: hàm kiểm tra xem có phải kết thúc nhập chữ trên composition
     */
    public boolean shouldFinishComposition(){
        return mWordComposer.isComposingWord();
    }

    /**
     * AnhNDd: kiểm tra xem ô text hiện tại có cho tự động sửa từ hay không
     */
    public boolean checkAutoCorrectionEnabled(SettingsValues settingsValues){
       return settingsValues.mAutoCorrectionEnabledPerUserSettings;
    }

    public void addToUserHistoryTyped(LastComposedWord lastComposedWord){
    }

    /**
     * AnhNDd: thêm kiểu gõ theo telex vào từ điển gõ
     */
    public void addToUserHistoryTyped(final NgramContext ngramContext){
    }

    /**
     * AnhNDd: thêm từ có thể sửa vào wordcomposer
     */
    public void setAutoCorrection(SuggestedWordInfo suggestedWordInfo){
        mWordComposer.setAutoCorrection(suggestedWordInfo);
    }

    /**
     * AnhNDd: xử lý thêm dấu cách sau từ gợi ý mà mình bấm vào
     */
    public void handleInsertAutomaticSpaceWhenPickSuggestion(SettingsValues settingsValues){
        mSpaceState = SpaceState.PHANTOM;
    }

    /**
     * AnhNDd: lấy ra số kí tự xóa tiếp theo nếu đã xóa được một lúc lâu
     */
    public int getLengthToDeleteAgain(int codePointBeforeCursorToDeleteAgain){
        return Character.isSupplementaryCodePoint(
                codePointBeforeCursorToDeleteAgain) ? 2 : 1;
    }

    /**
     * AnhNDd: xóa bao nhiêu kí tự sau khi bấm nút xóa
     */
    public int getLengthToDelete(int codePointBeforeCursor){
        return Character.isSupplementaryCodePoint(codePointBeforeCursor) ? 2 : 1;
    }

    /**
     * AnhNDd: xử lý update lại dải gợi ý khi thực hiện xóa chữ
     */
    public void updateSuggetOnBackPress(InputTransaction inputTransaction, int currentKeyboardScriptId){
        if (mConnection.hasSlowInputConnection()) {
            mSuggestionStripViewAccessor.setNeutralSuggestionStrip();
        } else if (inputTransaction.mSettingsValues.isSuggestionsEnabledPerUserSettings()
                && inputTransaction.mSettingsValues.mSpacingAndPunctuations
    ...
    /**
     * AnhNDd: Kiểm tra xem từ đã gõ có trong từ điển tiếng việt hay không
     * nếu không có thì thực hiện tự động sửa
     */
    public boolean checkWordTypedInMainVi(SuggestedWordInfo autoCorrectionOrNull){
        return true;
    }

    //AnhNDd: them flag
    protected void commitChosenWord(final SettingsValues settingsValues, final String chosenWord,
                                    final int commitType, final String separatorString, int flagAutoCorrect) {
        long startTimeMillis = 0;
        if (DebugFlags.DEBUG_ENABLED) {
            startTimeMillis = System.currentTimeMillis();
            Log.d(TAG, "commitChosenWord() : [" + chosenWord + "]");
        ...
        // 1st previous word is the word to be committed. Otherwise get n-gram context from the 1st
        // previous word.
        Log.d("phongnve", "commitChosenWord: "+settingsValues+" "+chosenWord);
        final NgramContext ngramContext = mConnection.getNgramContextFromNthPreviousWord(
                settingsValues.mSpacingAndPunctuations, mWordComposer.isComposingWord() ? 2 : 1);
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
        ...
        // Add the word to the user history dictionary
        performAdditionToUserHistoryDictionary(settingsValues, chosenWord, ngramContext, flagAutoCorrect); //AnhNDd them flagAutoCorrect
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
            Log.d(TAG, "commitChosenWord() : " + runTimeMillis + " ms to run "
                    + "performAdditionToUserHistoryDictionary()");
        ...
        // strings.
        mLastComposedWord = mWordComposer.commitWord(commitType,
                chosenWordWithSuggestions, separatorString, ngramContext, flagAutoCorrect); //Bkav AnhNDd
        if (DebugFlags.DEBUG_ENABLED) {
            long runTimeMillis = System.currentTimeMillis() - startTimeMillis;
            Log.d(TAG, "commitChosenWord() : " + runTimeMillis + " ms to run "
                    + "WordComposer.commitWord()");
    ...
    //AnhNDd: them flag
    public void performAdditionToUserHistoryDictionary(final SettingsValues settingsValues,
                                                       final String suggestion, @Nonnull final NgramContext ngramContext,int flagAutoCorrect) {
        // If correction is not enabled, we don't add words to the user history dictionary.
        // That's to avoid unintended additions in some sensitive fields, or fields that
        // expect to receive non-words.
        //if (!settingsValues.mAutoCorrectionEnabledPerUserSettings) return;
        ...
        //AnhNDd: Thêm từ vào từ điển cache, Nếu có flagAutoCorrect thì phải thêm cả telex, để thu thập dữ liệu xác định đây là từ sửa đúng
        final String cache;
        if (flagAutoCorrect != 0)
            cache = mWordComposer.getTypedWord() + CODE_SPERATOR + suggestion;
        else
            cache = suggestion;
        mDictionaryFacilitator.addToUserHistory(Dictionary.TYPE_CACHE_USER_HISTORY,cache, wasAutoCapitalized,
                ngramContext,0, settingsValues.mBlockPotentiallyOffensive, flagAutoCorrect);
        //Bkav AnhNDd: Thêm từ vào từ điển gõ telex nhưng chưa dùng
        addToUserHistoryTyped(ngramContext);
    }

    //Bkav AnhNDd dùng cho debug
    public String preCommitAutocorrect(String type, String commit){
        return commit;
    }
}