...
public class KeyboardBuilder<KP extends KeyboardParams> {
    ...
    private void parseKeyboardAttributes(final XmlPullParser parser) {
        ...
        try {
            ...
            //Bkav AnhNDd:
            final int baseWidth = createBaseWidth(params);
            ...
            //AnhNDd:
            params.mKeyVisualAttributes = createKeyVisualAttributes(keyAttr);
            ...
            //AnhNDd:
            params.mMaxMoreKeysKeyboardColumn = intitMaxMoreKeysKeyboardColumn(keyAttr);
    ...
    private KeyboardRow parseRowAttributes(final XmlPullParser parser)
        ...
        try {
            ...
            KeyboardRow keyboardRow = new KeyboardRow(mResources, mParams, parser, mCurrentY);
            //AnhNDd: kiểm tra xem có phải là hàng phím cuối cùng
            if(keyboardAttr.hasValue(R.styleable.Keyboard_isLastRow)){
                lastRow(keyboardRow);
            }
            return keyboardRow;
        } finally {
    ...
    private void parseKey(final XmlPullParser parser, final KeyboardRow row, final boolean skip)
        ...
        //AnhNDd:
        final Key key = createKey(keySpec, keyAttr, keyStyle, mParams, row);
    ...
    private boolean parseCaseCondition(final XmlPullParser parser) {
        ...
        try {
            ...
            final boolean keyboardThemeMacthed = matchTypedValue(caseAttr,
                    ...
                    /*KeyboardTheme.getKeyboardThemeName(mParams.mThemeId)*/getKeyboardThemeName(mParams.mThemeId));//AnhNDd
    ...
    /**
     * AnhNDd: trường hợp key này là ở hàng cuối của bàn phím
     */
    private boolean mBottomEdge = false;

    /**
     * AnhNDd: thực hiện khởi tạo đối tượng key
     */
    public Key createKey(@Nullable final String keySpec, @Nonnull final TypedArray keyAttr,
                         @Nonnull final KeyStyle style, @Nonnull final KeyboardParams params,
                         @Nonnull final KeyboardRow row){
        return new Key(keySpec,keyAttr,style,params,row);
    }

    /**
     * AnhNDd: thực hiện lấy ra tên của theme
     */
    public String getKeyboardThemeName(int id){
        return KeyboardTheme.getKeyboardThemeName(id);
    }
    ...
    /**
     * AnhNDd: lấy ra số cột tối đa ở bảng morekey
     */
    public int intitMaxMoreKeysKeyboardColumn(final TypedArray keyAttr){
        return keyAttr.getInt(
                R.styleable.Keyboard_Key_maxMoreKeysColumn, 5);
    }

    /**
     * AnhNDd: xử lý khi phát hiện đây là row cuối cùng
     */
    private void lastRow(KeyboardRow row){
        mBottomEdge = true;
    }

    /**
     * AnhNDd: số dùng để tính toán kích thước các phím và hàng phím của bàn phím
     */
    public int createBaseWidth(final KeyboardParams params){
        return params.mOccupiedWidth - params.mLeftPadding - params.mRightPadding;
    }
}
