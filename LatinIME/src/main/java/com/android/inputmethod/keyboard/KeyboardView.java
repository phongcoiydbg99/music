...
public class KeyboardView extends View {
    ...
    /*private*/protected final Drawable mKeyBackground;
    /*private*/protected final Drawable mFunctionalKeyBackground;
    /*private*/protected final Drawable mSpacebarBackground;
    private final float mSpacebarIconWidthRatio;
    /*private*/protected final Rect mKeyBackgroundPadding = new Rect();
    ...
    @Nullable
    /*private*/ protected Keyboard mKeyboard;
    @Nonnull
    ...
    public KeyboardView(final Context context, final AttributeSet attrs, final int defStyle) {
        ...
        //AnhNDd:
        mKeyVisualAttributes = createKeyVisualAttributes(keyAttr);
    ...
    public void setKeyboard(@Nonnull final Keyboard keyboard) {
        ...
        //AnhNDd:
        mKeyPreviewPopupOn = Settings.getInstance().getCurrent().mKeyPreviewPopupOn;
    }
    ...
    private void onDrawKeyboard(@Nonnull final Canvas canvas) {
        ...
        boolean m = this instanceof MoreKeysKeyboardView;
        ...
        if (drawAllKeys || isHardwareAccelerated) {
            ...
            for (final Key key : keyboard.getSortedKeys()) {
                ...
                //AnhNDd:
                drawSomeComponents(key, canvas, paint);
                //AnhNDd: drawnhung cai tu ve
                if (key.getLabel() == null)
                    drawIconEmoji(key, canvas);
            }
            //AnhNDd:
            drawNumberOnTop(canvas, mPaint);
        } else {
    ...
    /*private*/
    protected void onDrawKey(@Nonnull final Key key, @Nonnull final Canvas canvas,
                             @Nonnull final Paint paint) {
        //AnhNDd:
        if (mKeyPreviewPopupOn && key.isPressed() && ((key.getCode() >= 65 && key.getCode() <= 90)
                || (key.getCode() >= 97 && key.getCode() <= 122))) return;
        ...
        if (!key.isSpacer()) {
            ...
            //AnhNDd:
            final Drawable background = selectKeyBackground(key);
            if (background != null) {
    ...
    protected void onDrawKeyTopVisuals(@Nonnull final Key key, @Nonnull final Canvas canvas,
        ...
        if (label != null) {
            ...
            if (key.isEnabled()) {
                ...
                //AnhNDd:
                paint.setColor(getKeyTextColor(key, params));
            ...
            canvas.drawText(/*label*/displayUpperLabel(label), 0, label.length(), labelX, labelBaseline, paint);
    ...
    //AnhNDd: boolean kiểm tra xem có hiển thị preview hay không
    private boolean mKeyPreviewPopupOn;

    /**
     * AnhNDd: thực hiện vẽ một số thành phần cần thiết lên bàn phím
     */
    public void drawSomeComponents(Key key, Canvas canvas, Paint paint) {

    }

    /**
     * AnhNDd: thực hiện vẽ icon emoji lên phím enter
     */
    public void drawIconEmoji(Key key, Canvas canvas) {

    }

    /**
     * AnhNDd: Thực hiện vẽ hàng phím số trên phím bấm
     */
    public void drawNumberOnTop(Canvas canvas, Paint paint) {
    }

    /**
     * AnhNDd: setbackground cho key theo che do blur hay khong
     */
    public Drawable selectKeyBackground(Key key) {
        return key.selectBackgroundDrawable(
                mKeyBackground, mFunctionalKeyBackground, mSpacebarBackground);
    }

    /**
     * Hiển thị chữ hoa nếu cần
     */
    public String displayUpperLabel(String label) {
        return label;
    }

    /**
     * AnhNDd: khởi tạo KeyVisualAttributes
     */
    public KeyVisualAttributes createKeyVisualAttributes(@Nonnull TypedArray keyAttr) {
        return KeyVisualAttributes.newInstance(keyAttr);
    }

    /**
     * AnhNDd: lấy ra màu của label key để vẽ
     */
    public int getKeyTextColor(Key key, KeyDrawParams params) {
        return key.selectTextColor(params);
    }

    /**
     * AnhNDd: thay doi tint cho icon
     */
    public void setIconColor(Key key, Drawable icon) {
    }
}
