...
//AnhNDd:khoi tao bien toan cuc.
bool NOT_USE_FULL_SEARCH;
bool USE_DICT_AUTOCORRECT;
...
static void latinime_BinaryDictionary_getSuggestions(JNIEnv *env, jclass clazz, jlong dict,
    ...
    //AnhNDd: kiểm tra có dùng tất cả mọi trường hợp để lấy gợi ý hay không
    NOT_USE_FULL_SEARCH = givenSuggestOptions.notUseFullSearch();
    USE_DICT_AUTOCORRECT = givenSuggestOptions.useDictAutocorrect();
    //-------------
    ...
    if (givenSuggestOptions.isGesture() || inputSize > 0) {
        ...
        //AnhNDd: Với từ điển autocorrect không cần predict
...
static bool latinime_BinaryDictionary_updateEntriesForWordWithNgramContext(JNIEnv *env,
        ...
        jint timestamp, jint flagAutoCorrect) { //AnhNDd: CUSTOM_HISTORY
    Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
    if (!dictionary) {
        return false;
    }
    ...
    int wordCodePoints[codePointCount];
    env->GetIntArrayRegion(word, 0, codePointCount, wordCodePoints);
    const HistoricalInfo historicalInfo(timestamp, flagAutoCorrect /* level 0*/, count);//AnhNDd: CUSTOM_HISTORY
...
//AnhNDd: Hàm thực hiện lấy các char có thể gõ tiếp theo
    static void
    latinime_BinaryDictionary_getNextNodeSuggestedNative(JNIEnv *env, jclass clazz, jlong dict,
                                                         ...
                                                         jint prevWordCount,
                                                         jintArray outNodeSuggestionCount,
                                                         jintArray outNodeCodePointsArray,
                                                         jintArray outNodeScoresArray) {
        Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
        // Assign 0 to outSuggestionCount here in case of returning earlier in this method.
        JniDataUtils::putIntToArray(env, outNodeSuggestionCount, 0 /* index */, 0);
        if (!dictionary) {
            return;
        }
        DicTraverseSession *traverseSession =
        ...
        const jsize outputNodeCodePointsLength = env->GetArrayLength(outNodeCodePointsArray);
        if (outputNodeCodePointsLength != MAX_RESULTS) {
            AKLOGE("Invalid outputCodePointsLength: %d", outputNodeCodePointsLength);
            ASSERT(false);
            return;
        }
        const jsize scoresLength = env->GetArrayLength(outNodeScoresArray);
        if (scoresLength != MAX_RESULTS) {
            AKLOGE("Invalid scoresLength: %d", scoresLength);
            ASSERT(false);
            return;
        ...
        //AnhNDd: mảng để chứa giá trị trả về
        int *outputNodeCodePoints = new int[outputNodeCodePointsLength];
        int *outputNodeScores = new int[scoresLength];
        int outputNodeCount = 0;

        const NgramContext ngramContext = JniDataUtils::constructNgramContext(
                env, prevWordCodePointArrays, isBeginningOfSentenceArray, prevWordCount);

        if (inputSize > 0) {
            //AnhNDd: xử lý lấy node tiếp theo của từ đang gõ
            dictionary->getNextNodeSuggestions(traverseSession, inputCodePoints, inputSize,
                                               &ngramContext, &givenSuggestOptions,
                                               outputNodeCodePoints, outputNodeScores,
                                               &outputNodeCount);
        } else {
            //AnhNDd: xử lý lấy node sau khi gõ xong một từ
            //AnhNDd: Phải xử lý như thế này là do: Ở đây nó sẽ có một bảng buffer lưu xem sau các từ đã gõ thì sẽ là từ nào
            //Thì với mỗi từ đó sẽ có một id riêng, từ id đó thì lấy được terminal node, và có thể dịch ngược lại theo con trỏ
            //để lấy được từ gợi ý TODO: Hiện tại do đọc hiểu chưa kĩ nên làm tạm theo cách vẫn lấy nguyên từ gợi ý như thường
            //TODO: và xử lý để chỉ trả về mỗi codepoint đầu tiền, cách làm chưa hiệu quả cần xem lại
            //Tạm đóng do gây ra tỉ lệ gõ sai nhiều
            /*SuggestionResults suggestionResults(MAX_RESULTS);
            dictionary->getPredictions(&ngramContext, &suggestionResults);
            suggestionResults.outputNextNodePredictions(outputNodeCodePoints, outputNodeScores,
                                                        &outputNodeCount);*/
        }
        if (outputNodeCount > 0) {
            for (int i = 0; i < outputNodeCount; i++) {
                //AnhNDd: trả giá trị cho con trỏ trên java
                JniDataUtils::putIntToArray(env, outNodeCodePointsArray, i,
                                            outputNodeCodePoints[i]);
                JniDataUtils::putIntToArray(env, outNodeScoresArray, i, /*outputNodeScores[i]*/0); //Bkav AnhNDd: tạm thời để là 0
            }
            JniDataUtils::putIntToArray(env, outNodeSuggestionCount, 0 /* index */,
                                        outputNodeCount);
        }
    }

    //Bkav AnhNDd: kiểm tra xem 1 word có nằm trong từ điển không
    static bool latinime_BinaryDictionary_isInDictionary(JNIEnv *env, jclass clazz, jlong dict,
                                                         jintArray word) {
        Dictionary *dictionary = reinterpret_cast<Dictionary *>(dict);
        if (!dictionary) return NOT_A_PROBABILITY;
        const jsize codePointCount = env->GetArrayLength(word);
        int codePoints[codePointCount];
        env->GetIntArrayRegion(word, 0, codePointCount, codePoints);
        return dictionary->isInDictionary(CodePointArrayView(codePoints, codePointCount));
    }
...
static const JNINativeMethod sMethods[] = {
    ...
    //AnhNDd: khai báo hàm
    {
        const_cast<char *>("getNextNodeSuggestedNative"),
        const_cast<char *>("(JJ[II[I[[I[ZI[I[I[I)V"),
        reinterpret_cast<void *>(latinime_BinaryDictionary_getNextNodeSuggestedNative)
    },

    {
        const_cast<char *>("isInDictionaryNative"),
        const_cast<char *>("(J[I)Z"),
        reinterpret_cast<void *>(latinime_BinaryDictionary_isInDictionary)
    }
};
