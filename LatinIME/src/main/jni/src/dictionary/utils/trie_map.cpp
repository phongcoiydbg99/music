...
//AnhNDd: CUSTOM_HISTORY mình có mở lại thuộc tính level của historical_info.h. Mà giá trị max để check xem value add vào có thỏa mãn hay không bị thiếu 1 CHAR_BIT
//Nên tự cộng tay thêm 1 CHAR_BIT nữa
const uint64_t TrieMap::MAX_VALUE =
        (static_cast<uint64_t>(1) << ((FIELD0_SIZE + FIELD1_SIZE + 1) * CHAR_BIT)) - 1;
