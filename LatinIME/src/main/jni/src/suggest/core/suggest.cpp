...
namespace latinime {
    ...
    //AnhNDd
    bool isSpaceSubstitutionTerminal = false;
 ...
 */
    void Suggest::getSuggestions(ProximityInfo *pInfo, void *traverseSession,
        ...
        //AnhNDd:
        isSpaceSubstitutionTerminal = false;
 ...
 */
    void Suggest::expandCurrentDicNodes(DicTraverseSession *traverseSession) const {
        ...
        while (traverseSession->getDicTraverseCache()->activeSize() > 0) {
            DicNode dicNode;
            ALOGA("dicNode %d", dicNode.getNodeCodePoint());
            ...
            if (dicNode.isInDigraph()) {
                ...
                //AnhNDd:
                if (!NOT_USE_FULL_SEARCH && !USE_DICT_AUTOCORRECT) {
                ...
                //AnhNDd: Với tiếng việt chỉ mở rộng node trong trường hợp là từ điển dùng để tự động sửa từ
                if (!NOT_USE_FULL_SEARCH) {
                ...
                //Bkav AnhNDd: allowsErrorCorrections Cái ngưỡng điểm sửa sai SpatialCost vẫn chưa vượt quá ngưỡng đang là 0.95(NORMALIZED_SPATIAL_DISTANCE_THRESHOLD_FOR_EDIT)
                ...
                if (TRAVERSAL->isSpaceSubstitutionTerminal(traverseSession, &dicNode)) {
                    ...
                    //AnhNDd
                    if (!NOT_USE_FULL_SEARCH && !USE_DICT_AUTOCORRECT) { //Tạm đóng
                        isSpaceSubstitutionTerminal = true; // AnhNDd
                ...
                for (int i = 0; i < childDicNodesSize; ++i) {
                    ...
                    ALOGA("getNodeCodePoint %d",childDicNode->getNodeCodePoint());
                    if (isCompletion) {
                    ...
                    ALOGA("proximityType %d",proximityType);
 ...
 */
    void Suggest::processExpandedDicNode(
        ...
        //AnhNDd: doi voi tieng viet thi kiem tra xem dicnode co >= voi code point dau vao hay khong.
        if (NOT_USE_FULL_SEARCH) {
            if (dicNode->getNodeCodePointCount() >= traverseSession->getInputSize())
                processTerminalDicNode(traverseSession, dicNode);
        } else if (USE_DICT_AUTOCORRECT) { //Mở tất cả cho tự động sửa từ
            processTerminalDicNode(traverseSession, dicNode);
        } else
            processTerminalDicNode(traverseSession, dicNode);
        if (dicNode->getCompoundDistance() < static_cast<float>(MAX_VALUE_FOR_WEIGHTING)) {
...
// the normal non-digraph traversal, so both "uber" and "ueber" can be corrected to "[u-umlaut]ber".
    void Suggest::processDicNodeAsDigraph(DicTraverseSession *traverseSession,
        ...
        //AnhNDd: Mowr theem
        if (NOT_USE_FULL_SEARCH /*|| USE_DICT_AUTOCORRECT*/) return;
 ...
 */
    void Suggest::processDicNodeAsOmission(
        ...
        //AnhNDd: Mở tính năng gõ thiếu chữ
        if (NOT_USE_FULL_SEARCH /*|| USE_DICT_AUTOCORRECT*/) return;
 ...
 */
    void Suggest::processDicNodeAsInsertion(DicTraverseSession *traverseSession,
        ...
        //AnhNDd :Mở tính năng gõ thừa chữ
        if (NOT_USE_FULL_SEARCH /*|| USE_DICT_AUTOCORRECT*/) return;
 ...
 */
    void Suggest::processDicNodeAsTransposition(DicTraverseSession *traverseSession,
        ...
        //AnhNDd: Mở tính năng sửa từ gõ lộn vị trí
        if (NOT_USE_FULL_SEARCH /*|| USE_DICT_AUTOCORRECT*/) return;
            ...
            if (childDicNodes1[i]->hasChildren()) {
                ...
                for (int j = 0; j < childSize2; j++) {
                    ...
                    Weighting::addCostAndForwardInputIndex(WEIGHTING, CT_TRANSPOSITION,
                                                           traverseSession, childDicNodes1[i], childDicNode2,
                                                           0 /* multiBigramMap */);
 ...
 */
    ...
    //AnhNDd: Lấy ra kí tự tiếp theo có thể gõ dựa trên từ đang gõ
    void Suggest::getNextNodeSuggestions(void *traverseSession, int *inputCodePoints, int inputSize,
                                         int *outputNodeCodePoints, int *outputNodeScores, int *outputNodeCount) const {
        DicTraverseSession *tSession = static_cast<DicTraverseSession *>(traverseSession);
        tSession->setInputSize(inputSize);

        //AnhNDd: vị trí để duyệt các codepoint đầu vào
        int indexInputCodePoints = 0;
        if (inputSize >= MIN_CONTINUOUS_SUGGESTION_INPUT_SIZE
            && tSession->getDicTraverseCache()->hasCachedDicNodesForContinuousSuggestion()
            && tSession->getDicTraverseCache()->lastCachedInputIndex() == inputSize - 1) {
            //AnhNDd: lấy cache và vị trí duyệt
            tSession->getDicTraverseCache()->continueSearch();
            indexInputCodePoints = tSession->getDicTraverseCache()->lastCachedInputIndex();
        } else {
            tSession->resetCache(TRAVERSAL->getMaxCacheSize(tSession->getInputSize(),
                                                            tSession->getSuggestOptions()->weightForLocale()),
                                 TRAVERSAL->getTerminalCacheSize());
            //AnhNDd: khởi tạo node gốc
            DicNode rootNode;
            DicNodeUtils::initAsRoot(tSession->getDictionaryStructurePolicy(),
                                     tSession->getPrevWordIds(), &rootNode);
            tSession->getDicTraverseCache()->copyPushActive(&rootNode);
        }

        DicNodeVector childDicNodes(TRAVERSAL->getDefaultExpandDicNodeSize());
        while (tSession->getDicTraverseCache()->activeSize() > 0) {
            DicNode dicNode;
            tSession->getDicTraverseCache()->popActive(&dicNode);
            if (dicNode.isTotalInputSizeExceedingLimit()) {
                return;
            }
            childDicNodes.clear();
            ...
            DicNodeUtils::getAllChildDicNodes(
                    &dicNode, tSession->getDictionaryStructurePolicy(), &childDicNodes);
            const int childDicNodesSize = childDicNodes.getSizeAndLock();
            for (int i = 0; i < childDicNodesSize; ++i) {
                DicNode *const childDicNode = childDicNodes[i];
                if(childDicNode->getNodeCodePoint() == inputCodePoints[indexInputCodePoints]){
                    //AnhNDd: tìm được node giống với node đang nhập -> mở rộng node đó
                    if(childDicNode->hasChildren()) {
                        indexInputCodePoints++; //AnhNDd: duyệt node tiếp theo
                        tSession->getDicTraverseCache()->copyPushActive(childDicNode);

                        //AnhNDd: đến node có thể set cache
                        if(indexInputCodePoints == inputSize){
                            tSession->getDicTraverseCache()->copyPushContinue(childDicNode);
                            tSession->getDicTraverseCache()->setLastCachedInputIndex(inputSize);
                            childDicNode->setCached();
                        }
                        break;
                    }
                }else{
                    //AnhNDd: Trong trường hợp không phải và ở node đó có độ dài lớn hơn độ dài từ đang gõ
                    //thì node này là node có thể gõ tiếp theo
                    //AnhNDd: chỉ lấy ra 18 node
                    int codePoint = childDicNode->getNodeCodePoint();
                    if (childDicNode->getNodeCodePointCount() > inputSize && *outputNodeCount < MAX_RESULTS &&
                            codePoint >= 'a' && codePoint <= 'z') { //Bkav AnhNDd: do từ điển auto correct có từ tiếng việt nên cần check
                        outputNodeCodePoints[*outputNodeCount] = codePoint;
                        //AnhNDd: khi có node tiếp theo cần tính điểm cho node đó
                        outputNodeScores[*outputNodeCount] = getNextNodeScore(tSession, childDicNode,
                                                                              inputSize);
                        (*outputNodeCount)++;
                    }
                }
    ...
    //AnhNDd: tính điểm cho node hiện tại chỉ dùng trong mở rộng phím
    //Điểm của node là tổng điểm của các từ tiếng việt có thể tạo được từ node đó
    int Suggest::getNextNodeScore(void *traverseSession, DicNode *dicNode, int inputSize) const {
        DicTraverseSession *tSession = static_cast<DicTraverseSession *>(traverseSession);
        if (dicNode->isTerminalDicNode()) {
            //AnhNDd: lấy ra điểm của từ đó
            const WordAttributes wordAttributes =
                    tSession->getDictionaryStructurePolicy()->getWordAttributesInContext(
                            tSession->getPrevWordIds(), dicNode->getWordId(),
                            tSession->getMultiBigramMap());
            return wordAttributes.getProbability();
        } else {
            int score = 0;
            int maxScore = 0;
            DicNodeVector childNextDicNodes(60); //AnhNDd: số node tối đa
            DicNodeUtils::getAllChildDicNodes(
                    dicNode, tSession->getDictionaryStructurePolicy(), &childNextDicNodes);
            const int childNextDicNodesSize = childNextDicNodes.getSizeAndLock();
            for (int i = 0; i < childNextDicNodesSize; i++) {
                maxScore = getNextNodeScore(traverseSession, childNextDicNodes[i], inputSize);
