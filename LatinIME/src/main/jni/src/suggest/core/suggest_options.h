...
class SuggestOptions{
 public:
    ...
    //AnhNDd:
    AK_FORCE_INLINE bool notUseFullSearch() const {
        return getBoolOption(NOT_USE_FULL_SEARCH);
    }

    //AnhNDd:
    AK_FORCE_INLINE bool useDictAutocorrect() const {
        return getBoolOption(USE_DICT_AUTOCORRECT);
    }
 ...
 private:
    ...
    //AnhNDd:Thêm option lựa chọn search full
    static const int NOT_USE_FULL_SEARCH = 6;

    //AnhNDd: thêm option search theo từ điển autocorrect
    static const int USE_DICT_AUTOCORRECT = 7;
