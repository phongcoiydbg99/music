...
class Dictionary {
 public:
    ...
    //AnhNDd:
    void getNextNodeSuggestions(DicTraverseSession *traverseSession, int *inputCodePoints,
                        int inputSize, const NgramContext *const ngramContext, const SuggestOptions *const suggestOptions,
    ...
    //Bkav AnhNDd: hàm kiểm tra xem word đã có trong từ điển hay không
    bool isInDictionary(const CodePointArrayView codePoints) const;
