...
void Dictionary::logDictionaryInfo(JNIEnv *const env) const {
    ...
    //AnhNDd:
    void Dictionary::getNextNodeSuggestions(DicTraverseSession *traverseSession, int *inputCodePoints, int inputSize,
        ...
        TimeKeeper::setCurrentTime();
        traverseSession->init(this, ngramContext, suggestOptions);
        const auto &suggest = mTypingSuggest;
        suggest->getNextNodeSuggestions( traverseSession,inputCodePoints, inputSize, outputNodeCodePoints,outputNodeScores,outputNodeCount);
    }

    //Bkav AnhNDd
    bool Dictionary::isInDictionary(const latinime::CodePointArrayView codePoints) const {
        const int wordId = mDictionaryStructureWithBufferPolicy->getWordId(codePoints, false);
