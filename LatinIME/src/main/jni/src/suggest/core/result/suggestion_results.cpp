...
void SuggestionResults::dumpSuggestions() const {
    ...
    //AnhNDd: xử lý hàm
    void SuggestionResults::outputNextNodePredictions(int *outputNodeCodePoints, int *outputNodeScores, int *outputNodeCount) {
        while (!mSuggestedWords.empty()) {
            const SuggestedWord &suggestedWord = mSuggestedWords.top();
            //AnhNDd: những gợi ý có cùng chữ cái bắt đầu thì nhóm làm một cặp
            int firstCodePoint = suggestedWord.getCodePoint()[0];
            if(*outputNodeCount == 0) {
                outputNodeCodePoints[0] = firstCodePoint;
                outputNodeScores[0] = suggestedWord.getScore();
                (*outputNodeCount)++;
                mSuggestedWords.pop();
                continue;
            }
            for(int i=0;i<*outputNodeCount;i++){
                if(firstCodePoint == outputNodeCodePoints[i]){
                    outputNodeScores[i]+= suggestedWord.getScore();
                }else{
                    outputNodeCodePoints[*outputNodeCount] = firstCodePoint;
                    outputNodeScores[*outputNodeCount] = suggestedWord.getScore();
                    (*outputNodeCount)++;
                    break;
                }
            }
