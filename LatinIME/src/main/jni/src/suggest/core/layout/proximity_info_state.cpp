...
ProximityType ProximityInfoState::getProximityType(const int index, const int codePoint,
    ...
    const int baseLowerC = CharUtils::toBaseLowerCase(codePoint);
     ...
     if (NOT_USE_FULL_SEARCH) {
           //AnhNDd:Kiem tra them truong hop
           const int baseCharVN = CharUtils::toBaseCharVN(codePoint);
           if (firstCodePoint == baseLowerC || firstCodePoint == codePoint || firstCodePoint == baseCharVN)
               return MATCH_CHAR;
    }
    ...
    //AnhNDd: chi xu ly voi match_char, khong thi khong xu ly
    if (NOT_USE_FULL_SEARCH){
    ...
    while (j < MAX_PROXIMITY_CHARS_SIZE
        ...
        //AnhNDd: autocorrect mới chỉ xử lý cho việc bấm nhầm chữ
        if (USE_DICT_AUTOCORRECT){
            return UNRELATED_CHAR;
        }
