...
/* static */ float AutocorrectionThresholdUtils::calcNormalizedScore(const int *before,
    ...
    if (score <= 0 || distance >= afterLength) {
        ...
        //AnhNDd: distance tổng các kí tự khác với từ tự động sửa
        // nếu distance nhỏ hơn thì mới có khả năng để xử lý từ đó, vì hiện tại mới chỉ tự động sửa cho từ telex
        if(USE_DICT_AUTOCORRECT && distance > beforeLength){
            return 0.0f;
        }
